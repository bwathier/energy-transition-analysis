...
# Generate best-model forecast plots for energy categories (interactive)
# =====================================================================


# ---------------------------------------------------------------------
# Extract forecast object for a given category + model family
# ---------------------------------------------------------------------
extract_category_forecast <- function(category_name, model_name, forecast_results) {
  model_name <- tolower(model_name)
  
  if (!model_name %in% c("arima", "ets", "prophet")) return(NULL)
  
  family_obj <- forecast_results[[model_name]]
  if (is.null(family_obj) || !"forecast_categories" %in% names(family_obj)) return(NULL)
  
  fc_tbl <- family_obj$forecast_categories
  fc_obj <- fc_tbl %>% filter(category == category_name)
  
  if (nrow(fc_obj) == 0) return(NULL)
  fc_obj
}
...
# ---------------------------------------------------------------------
add_intervals_if_missing_categories <- function(df) {
  ...
  if ("consumption" %in% names(df) && is.numeric(df$consumption)) {
    resid    <- df$consumption - df$.mean
    resid_sd <- stats::sd(resid, na.rm = TRUE)
    
    if (is.finite(resid_sd) && resid_sd > 0) {
      df$.lower_eff <- df$.mean - 1.96 * resid_sd
      df$.upper_eff <- df$.mean + 1.96 * resid_sd
      return(df)
    }
  }
...
# ---------------------------------------------------------------------
# Main category plotting function (interactive)
# ---------------------------------------------------------------------
plot_best_forecasts_categories <- function(best_models,
                                           forecast_results,
                                           category_colors,
                                           config) {
  ...
  out_dir         <- config$forecast$paths$output_categories
  consumption_units <- config$plot_theme$labels$consumption_units %||% "Quadrillion Btu"
  
  dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
  
  category_rows <- best_models %>% filter(Type == "Category")
  
  for (i in seq_len(nrow(category_rows))) {
    
    row         <- category_rows[i, ]
    category_nm <- row$Description
    model_name  <- row$Best_Model
    
    fc_obj <- extract_category_forecast(category_nm, model_name, forecast_results)
    if (is.null(fc_obj)) next
    
    df <- as_tibble(fc_obj)
    df$date <- as.Date(df$date)
    df <- add_intervals_if_missing_categories(df)
    
    df <- df %>% arrange(date)
    df$t <- as.numeric(df$date)
    
    color <- category_colors[[category_nm]] %||% "#2C3E50"
    ...
    horizon_end_date <- as.Date(paste0(today_year + horizon_years, "-12-31"))
    end_date        <- min(forecast_end, horizon_end_date)
    ...
    title_text <- paste0("Category Forecast: ", category_nm)
    ...
    safe_name <- stringr::str_replace_all(category_nm, "[^A-Za-z0-9]+", "_")
    
    saveWidget(
      p,
      file = file.path(out_dir, paste0("forecast_category_", safe_name, ".html")),
      selfcontained = TRUE
    )
  }
}
